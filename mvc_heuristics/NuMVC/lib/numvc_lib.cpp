...
int Numvc(const int v_num, const int e_num, const int* edges_from, const int* edges_to,  int* mis, const float cutoff_time)
{
  ::v_num = v_num;
  ::e_num = e_num;
  ::cutoff_time = cutoff_time;
  ::optimal_size = 0;
  if(build_instance_from_graph(v_num, e_num, edges_from, edges_to) != 1){
    cout << "c Graph not built... " << endl;
    free_memory();
    return -1;
  }
